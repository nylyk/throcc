...
const TLS_CHANNEL_BINDING: [u8; 32] = [2u8; 32];
...
fn refusal(decision: Decision) -> AuthError {
    match decision {
        ...
        Decision::Admitted { .. } => panic!("expected a refusal"),
    }
}

fn admitted(decision: Decision) -> User {
    match decision {
        Decision::Admitted { user, .. } => user,
        Decision::Refused(error) => panic!("expected an admission, got {error:?}"),
    }
}
...
fn a_signature_is_bound_to_this_connection_and_this_nonce() {
    ...
    let code = state.database.create_invite().unwrap().code;

    let relayed = signed(&key, &SERVER_NONCE, &[9u8; 32], Some(&code));
    let decision =
        auth::decide(&state, &relayed, &SERVER_NONCE, &TLS_CHANNEL_BINDING, PEER).unwrap();
    assert_eq!(
        refusal(decision),
        AuthError::BadSignature,
        "a signature made against another connection's tls_channel_binding must not be accepted"
    );

    let replayed = signed(&key, &[8u8; 32], &TLS_CHANNEL_BINDING, Some(&code));
    let decision =
        auth::decide(&state, &replayed, &SERVER_NONCE, &TLS_CHANNEL_BINDING, PEER).unwrap();
...
fn a_code_enrolls_once() {
    ...
    let decision = auth::decide(
        &state,
        &signed(&first, &SERVER_NONCE, &TLS_CHANNEL_BINDING, Some(&code)),
        &SERVER_NONCE,
        &TLS_CHANNEL_BINDING,
        PEER,
    )
    .unwrap();
    let enrolled = admitted(decision);
    assert_eq!(
        enrolled.role,
        Role::Admin,
        "the first key to enrol carries the deployment's only Admin"
    );

    let second = SigningKey::generate(&mut rand::rng());
    let decision = auth::decide(
        &state,
        &signed(&second, &SERVER_NONCE, &TLS_CHANNEL_BINDING, Some(&code)),
        &SERVER_NONCE,
        &TLS_CHANNEL_BINDING,
        PEER,
    )
...
fn a_lower_case_code_is_accepted() {
    ...
    .unwrap();
    assert_eq!(admitted(decision).role, Role::Admin);
}
...
fn guessing_stops_being_attempted_once_the_budget_is_spent() {
    let directory = TempDir::new().unwrap();
    let state = state(&directory);
    let valid = state.database.create_invite().unwrap().code;
    ...
    let decision = auth::decide(
        &state,
        &signed(&guesser, &SERVER_NONCE, &TLS_CHANNEL_BINDING, Some(&valid)),
        &SERVER_NONCE,
        &TLS_CHANNEL_BINDING,
        PEER,
    )
