...
/// The only targeting rule: a role may act on strictly lower ranks. Every
/// mutating handler goes through here rather than comparing roles itself.
...
/// The capability half: a role carries the powers of its own rank, which the
/// rank ordering cannot express on its own.
