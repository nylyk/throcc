...
async fn main() -> Result<()> {
    ...
    if let Some(code) = server.mint_bootstrap_invite()? {
        let hours = throcc_server::invite::TTL.as_secs() / 3600;
        tracing::info!(
            "\n\nRedeem invite code  {code}  to enroll the first administrator.\nThe code is single use and expires in {hours} hours.\n"
        );
    }
