...
/// A fresh code of six symbols, worth roughly 31 bits.
...
/// budget backs it up. Exceeding it stops all redemption until a restart.
        ...
        if self.total_failures == SERVER_WIDE_FAILURE_BUDGET {
            tracing::error!(
                "the server-wide invite failure budget is exhausted; \
                 redemption is refused until this server restarts"
            );
        }
    }
}
